...
| 行为 | 说明 | 应对方式 |
|------|------|---------|
| `//` 表示层级分隔 | 标签名中的 `//` 是层级分隔符，写入 `#父级//子级` 时 WPS 自动将其解析为父子关系。路径中必须用 `//` 分隔层级，单个 `/` 会被当作普通字符 | 写标签时直接写完整路径（如 `#工作//项目`），不要把父级名和子级名分段拼接后再合并 |
...
**必须验证两个层面：**
1. **笔记块内容**（`read_blocks`）：标签是否正确写入，是否已成为 `<tag>` 元素，block 内其他内容是否完整；**检查层级标签使用 `//`，且不要误写成单个 `/`**
...
| 错误 | 后果 | 正确做法 |
|------|------|---------|
| 层级标签误写成单个 `/`，如 `#工作/项目` | 系统无法按当前约定解析成父子标签，检索和展示都会偏差 | 层级路径统一使用 `//`，写入前检查为 `#工作//项目` 这种完整路径 |
