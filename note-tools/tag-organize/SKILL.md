...
| 行为 | 说明 | 应对方式 |
|------|------|---------|
| `/` 表示层级分隔 | 多级标签用**单个** `/` 连接各级，如 `#父级/子级`；WPS 会解析为父子关系。**不要使用 `//`**（双斜杠），否则层级解析与检索会与预期不符 | 写 `<tag>#工作/项目</tag>` 这类完整路径；与 WPS 笔记 MCP 中 `<tag>` 约定一致 |
...
**必须验证两个层面：**
1. **笔记块内容**（`read_blocks`）：标签是否正确写入，是否已成为 `<tag>` 元素，block 内其他内容是否完整；**层级只用单个 `/`（如 `#工作/项目`），不要写成 `//`**
...
| 错误 | 后果 | 正确做法 |
|------|------|---------|
| 层级标签误用 `//`，如 `#工作//项目` | 与 WPS 约定不符，父子关系解析、筛选可能异常 | 多级只用一个 `/`：`<tag>#工作/项目</tag>`；勿用双斜杠 |
