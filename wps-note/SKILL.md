...
| 工具 | 用途 | 关键参数 |
|------|------|----------|
| `list_notes` | 列出笔记，支持排序和分页，返回 { notes, meta } | `limit?`、`sort?`、`direction?`、`since?`、`before?`、`page?`、`page_size?` |
| `create_note` | 创建空白笔记（仅含空段落），返回 { fileId, title } | `title?` |
| `get_note_info` | 获取笔记元数据（不读全文），返回 title、时间、intro、tags | `note_id`（单个）、`note_ids`（批量）、`limit?`、`page`+`page_size`（分页） |
