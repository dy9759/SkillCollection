...
- **场景 Skill**：如新闻解读、标签整理、笔记打磨、实时转写总结、编码助手、内容创作、公众号发布、智能深度搜索等，复用 `wps-note` 的基础能力，不重复定义底层操作
...
wpsnote-beautifier/              # 场景：智能美化笔记文档，统一配色与排版
  SKILL.md
  reference/
    color-schemes.md
    xml-patterns.md
...
content-creator/                 # 场景：专业内容创作工作流，从0到1完成文章创作
  SKILL.md
  examples/
  references/
  scripts/

wechat-publisher/                # 场景：公众号排版发布，将笔记导出为微信公众号 HTML
  SKILL.md
  references/
  scripts/
  templates/

wps-note-intelligent-search/     # 场景：智能深度搜索，跨笔记关联挖掘与知识图谱构建
  SKILL.md
  evals/
  examples/
  references/
...
智能美化 WPS 笔记文档，采用克制统一的配色风格。核心能力包括：优化标题层级结构、用高亮块强调核心结论与注意事项、用分栏展示对比或并列内容、应用统一配色方案并写入。

- [Skill 定义](./wpsnote-beautifier/SKILL.md)
...
专业内容创作工作流，覆盖从0到1的完整创作流程：风格画像→需求确认→大纲设计→深度研究→文章创作→定稿交付。当用户说"帮我写""我想写""写公众号""写小红书""仿照这个风格写"时触发。与 `wechat-publisher` 形成完整创作闭环。

- [Skill 定义](./content-creator/SKILL.md)

### `wechat-publisher`

公众号发布助手，将已完成的 WPS 笔记内容一键排版并导出为微信公众号 HTML 格式。支持多种模板风格（blue-theme、default、minimal、elegant），提供占位符标签快速排版能力。这是创作流程的最后一步，与 `content-creator` 配合使用。

- [Skill 定义](./wechat-publisher/SKILL.md)

### `wps-note-intelligent-search`

【深度搜索】深挖笔记关联，构建知识图谱的 WPS 笔记查询助手。当用户需要找"张总的所有信息""Q3相关内容""整理待办""最近关于前端的笔记"时使用。支持跨笔记关联、语义联想、知识沉淀，不同于简单关键词搜索。

- [Skill 定义](./wps-note-intelligent-search/SKILL.md)
...
`news-to-note`、`tag-organize`、`note-copilot`、`live-transcript-summary`、`coding-assistant`、`literature-reader`、`wpsnote-beautifier`、`content-creator`、`wechat-publisher`、`wps-note-intelligent-search` 等目录聚焦具体任务流程，复用 `wps-note` 的 MCP 或 CLI 能力完成业务目标。
